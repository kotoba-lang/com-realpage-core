(ns realpage_core.main
  "Kotodama WASM entrypoint for the realpage_core clean-room actor (L4) — Clojure port.
  ...
  facts (`realpage_core.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "realpage_core")
(def tier "L4")
...
(def entity-specs
  [{:entity "Listing"      :plural "listings"      :id-prefix "realpage_lis"
   ...
   {:entity "Tenant"       :plural "tenants"       :id-prefix "realpage_ten"
...
(defn emit-facts
  "EAVT facts for one record: {\"realpage_core.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "realpage_core-compat" :tier tier :entities entities} 200])
...
(defn start! [] :realpage_core-compat/ready)
